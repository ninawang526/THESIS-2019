...
# simple LDA comparison of topic distributions among a given article set
def LDA_consistency(article_set, words):
	articles, dictionary, corpus, model = lex.LDA(article_set, words, num_topics=20) #30?
	...
	distros = []
	for label, articleset in bowarts.items():
		distros.append(ldac.total_topic_distribution(model, articleset))
		print ("getting distro for %s" %(label))
	jsd = ldac.compare_topic_distributions(distros)
	print("JS Divergence of topic distributions: %f"%(jsd))


#
def DTM_consistency(article_set, words):
